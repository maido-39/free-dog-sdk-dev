...
##   Initialize variables container     ##
qInit = [0] * jointnum
qDes = [0] * jointnum
qCurrent = [0] * jointnum  # Current motor positions
...
## Stand-up Motion Parameters
standuptime = 10.0  # Stand-up motion duration in seconds
smooth_transition_time = 3.0  # Smooth transition from current position to first stand-up position (seconds)
standup_torque = [-0.65, 2, 2, -0.65, 2, 2, -0.65, 2, 2, -0.65, 2, 2]  # Torque values for stand-up motion

## Stand-up Position List (can be customized)
# Each element represents a set of joint positions for different phases of stand-up motion
rad_standup_pos_list = []

# Create dummy stand-up positions (3 phases)
for phase in range(3):
    phase_positions = []
    for i in range(jointnum):
        if i % 3 == 0:  # XX_0 joints (hip)
            phase_positions.append(0)  # Hip joints stay at 0
        else:  # XX_1, XX_2 joints (thigh, calf)
            # Gradually increase angle for each phase
            phase_ratio = (phase + 1) / 3.0
            angle = functionGenerator(0, freq_hz_list[i], deg_sineamplitude[i] * phase_ratio, deg_sinemedian[i], phase_delay_list[i])
            phase_positions.append(angle)
    rad_standup_pos_list.append(phase_positions)

print(f"Stand-up motion duration: {standuptime} seconds")
print(f"Smooth transition time: {smooth_transition_time} seconds")
print(f"Number of stand-up phases: {len(rad_standup_pos_list)}")

# Convert standuptime to motiontime cycles
standup_cycles = int(standuptime / ctrldt)
smooth_transition_cycles = int(smooth_transition_time / ctrldt)

# Calculate cycles per stand-up phase
cycles_per_phase = standup_cycles // len(rad_standup_pos_list)

print(f"Stand-up cycles: {standup_cycles}")
print(f"Smooth transition cycles: {smooth_transition_cycles}")
print(f"Cycles per phase: {cycles_per_phase}")
...
try:
    ...
    while True:
        ...
        if( motiontime >= 0):
            ...
            ## Phase 2: Smooth transition to first stand-up position
            elif( motiontime >= 10 and motiontime < (10 + smooth_transition_cycles)):
                transition_rate = (motiontime - 10) / smooth_transition_cycles
                transition_rate = np.fmin(np.fmax(transition_rate, 0.0), 1.0)
                
                # Smooth transition from current position to first stand-up position
                for joint_idx in range(len(joint)):
                    qDes[joint_idx] = jointLinearInterpolation(qInit[joint_idx], rad_standup_pos_list[0][joint_idx], transition_rate)
                
                print(f"Smooth transition rate: {transition_rate:.3f}")

            ## Phase 3: Stand-up motion through different phases
            elif( motiontime >= (10 + smooth_transition_cycles) and motiontime < (10 + smooth_transition_cycles + standup_cycles)):
                standup_progress = motiontime - (10 + smooth_transition_cycles)
                
                # Determine which phase we're in
                current_phase = min(int(standup_progress / cycles_per_phase), len(rad_standup_pos_list) - 1)
                next_phase = min(current_phase + 1, len(rad_standup_pos_list) - 1)
                
                # Calculate interpolation rate within current phase
                phase_progress = (standup_progress % cycles_per_phase) / cycles_per_phase
                
                # Interpolate between current and next phase positions
                for joint_idx in range(len(joint)):
                    current_pos = rad_standup_pos_list[current_phase][joint_idx]
                    next_pos = rad_standup_pos_list[next_phase][joint_idx]
                    qDes[joint_idx] = jointLinearInterpolation(current_pos, next_pos, phase_progress)
                
                print(f"Stand-up phase: {current_phase}/{len(rad_standup_pos_list)-1}, progress: {phase_progress:.3f}")


            ## Phase 4: Maintain final stand-up position
            else:
                # Maintain the final stand-up position
                final_position = rad_standup_pos_list[-1]
                for joint_idx in range(len(joint)):
                    qDes[joint_idx] = final_position[joint_idx]
                
                print("Maintaining final stand-up position")
            ...
            # Data logging for tracking performance
            if motiontime >= 10:  # Start logging after initial positioning
    ...
    if len(target_positions) > 0 and len(actual_positions) > 0:
        ...
        ensure_logs_directory()
        csv_filename = get_timestamp_filename('standup_tracking_data', 'csv')
        plot_filename = get_timestamp_filename('standup_tracking_analysis', 'png')
...
# 사용 예시:
# plot_joint_tracking_from_csv('logs/2024-01-15_14-30-25_standup_tracking_data.csv', 'logs/analysis_plot.png')
# 
# 최신 CSV 파일 자동 분석:
...
#
# Stand-up motion parameters can be customized:
# standuptime = 15.0  # Change stand-up duration
# smooth_transition_time = 5.0  # Change smooth transition time
# standup_torque = [custom_torque_values]  # Change torque values
